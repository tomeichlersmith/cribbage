...
fn main() {
    ...
    let tot_hands : u64 = if let Some(max_hands) = args.max_hands { max_hands } else { 12_994_800 };
    let progbar = indicatif::ProgressBar::new(tot_hands);
    for hand in deck::full().iter().copied().combinations(4).map(|cards| Hand { hand : cards }) {
